...
class UI {
    ...
    buttons.forEach(button => {
      button.addEventListener("click", e => {
        ...
        if(cartItem && cartItem.info){
          ...
          if(cartItem.info.ram){
            html = html + `<div class="view-info">RAM: ${cartItem.info.ram}</div>`
          }
          if(cartItem.info.diskdrive){
            html = html + `<div class="view-info">Ổ cứng: ${cartItem.info.diskdrive}</div>`
          }
          if(cartItem.info.vga){
            html = html + `<div class="view-info">vga: ${cartItem.info.VGA}</div>`
          }
          if(cartItem.info.computerscreen){
            html = html + `<div class="view-info">Màn hình: ${cartItem.info.computerscreen}</div>`
          }
          if(cartItem.info.OS){
            html = html + `<div class="view-info">Hệ điều hành: ${cartItem.info.OS}</div>`
          }
          if(cartItem.info.color){
            html = html + `<div class="view-info">Màu: ${cartItem.info.Color}</div>`
          }
  ...
  confirmOrder() {
    if (confirm("Bạn phải đăng nhập trước khi đặt hàng!")) {
      // alert('Suscessfully!')
      // this.clearCart();
      this.hide();
    } else {
...
class Products {
  async getProducts() {
    try {
      const result = await fetch("../../json/laptop.json");
